    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                $deleteBtn = "<a href='".route('admin.products-image-gallery.destroy', $query->id)."' class='ml-3 mt-2 btn btn-danger delete-item'> delete </a>";
                return $deleteBtn;
            })
            ->addColumn('image', function($query){
                return "<img width='250px' src='".asset($query->image)."' ></img>";
            })
