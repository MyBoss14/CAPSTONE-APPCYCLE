...
<div class="form-group wsus__input mt-4" hidden>
    <label for="materials"> Category </label>
    <select id="materialSelect" name="material">
        <option value=""></option>
        <option {{$product->category_id == 20 ? 'selected' : ''}} value="metal">Metal</option>
        <option {{$product->category_id == 19 ? 'selected' : ''}} value="ewaste">Ewaste</option>
        <option {{$product->category_id == 21 ? 'selected' : ''}} value="textile">Textile</option>
    </select>
...
<script type="text/javascript">
      ...
      async function predict() {
        ...
        if (bestMatch.className.trim() === selectedMaterial) {
          ...
          toastr.success('<br>UPLOAD BUTTON IS NOW AVAILABLE', 'SUCCESS!', {
                            positionClass: 'toast-center',
                            });
        } else {
          document.getElementById("submitButton").hidden = true;
          toastr.error("INVALID UPLOAD <br> CATEGORY AND ITEM DOESN'T MATCH <br>PLEASE REFRESH AND PROVIDE A CLEARER IMAGE OR SUBMIT FOR ADMIN APPROVAL", "REFRESH THE PAGE", {
                            positionClass: 'toast-center',
                            });
        }
